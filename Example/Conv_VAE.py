...
class ConvVae(Model):
    ...
    def _generate_train_batch(self):
        _, self.batch_x = self.data.generate_train_batch(self.flags['batch_size'])
    ...
    def _run_train_summary_iter(self):
        ...
        self.summary, self.loss, self.x_recon, _ =\
            self.sess.run([self.merged, self.cost, self.x_hat, self.optimizer],
                          feed_dict={self.x: self.batch_x,self.keep_prob: 1.0,self.epsilon: norm,self.lr: 0.01})
...
def main():
    ...
    model_vae = ConvVae(flags, run_num=1)
    model_vae.train()
